...
_SARVAM_MAX_CHARS = 490


def _split_tts_text(text: str, max_chars: int = _SARVAM_MAX_CHARS) -> list[str]:
    """Split text into chunks ≤ max_chars, preferring sentence/line boundaries.
    Always normalises newlines → sentence separators so Sarvam doesn't stop early."""
    # Normalise newlines first (Sarvam TTS stops at raw newline chars)
    flat = text.replace("\n", ". ").replace("..", ".").strip()
    ...
    for sent in sentences:
        ...
        if len(current) + len(sent) + 1 <= max_chars:
            ...
            if len(sent) <= max_chars:
                ...
                # Hard-split overlong sentence at word boundary
                words = sent.split()
                current = ""
                for w in words:
...
def _concat_wav(wav_list: list[bytes]) -> bytes:
    """Concatenate multiple PCM WAV files into one by merging PCM data."""
    import struct
    if not wav_list:
        return b""
    if len(wav_list) == 1:
        return wav_list[0]
    # WAV standard header is 44 bytes; PCM data starts at byte 44
    pcm_data = b"".join(w[44:] for w in wav_list if len(w) > 44)
    if not pcm_data:
        return wav_list[0]
    header = bytearray(wav_list[0][:44])
    struct.pack_into("<I", header, 4, len(pcm_data) + 36)   # RIFF chunk size
    struct.pack_into("<I", header, 40, len(pcm_data))        # data sub-chunk size
...
def _sarvam_synthesise(text: str, language: str = "en-IN") -> bytes:
    ...
    Long texts are split into ≤490-char chunks and the resulting WAV files
    are concatenated so the full response is always spoken.
    """
    import base64
    try:
    ...
    for chunk in chunks:
        ...
        if not audios:
            raise RuntimeError(f"Sarvam returned no audio for chunk: {data}")
        wav_parts.append(base64.b64decode(audios[0]))

    if not wav_parts:
        raise RuntimeError("Sarvam produced no audio parts")
