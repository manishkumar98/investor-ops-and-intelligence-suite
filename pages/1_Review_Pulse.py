...
if dashboard_file.exists():
    st.markdown("---")
    st.markdown("**Queue MCP Actions for Approval**")
    st.caption("Sends pulse + fee context to the Approval Center (Tab 3).")

    if st.button("📤 Queue notes_append + email_draft", type="primary"):
        from phase7_pillar_c_hitl.mcp_client import enqueue_action
        from datetime import date

        pulse_data = _load_json("pulse_latest.json") or {}
        fee_data   = _load_json("fee_latest.json") or {}
        top_3      = pulse_data.get("top_3_themes", [])
        note_text  = pulse_data.get("weekly_note", "")
        ...
        enqueue_action(
            ...
            payload={
                ...
                "entry": {
                    "date":         str(date.today()),
                    "weekly_pulse": note_text[:500],
                    "top_themes":   top_3,
                    "fee_scenario": fee_data.get("scenario", ""),
                },
            },
            source="review_pulse_dashboard",
        )
        enqueue_action(
            st.session_state,
            type="email_draft",
            payload={
                ...
                "body": (
                    f"Weekly Pulse:\n{note_text}\n\n"
                    f"Fee Context:\n" + "\n".join(fee_data.get("bullets", []))
                    + f"\n\nSources: {', '.join(fee_data.get('sources', []))}"
                ),
            },
            source="review_pulse_dashboard",
        )
        ...
        st.success("✅ 2 actions queued — open Approval Center to approve")

    pending = [a for a in st.session_state.get("mcp_queue", []) if a.get("status") == "pending"]
    if pending:
        st.markdown(f"**{len(pending)} pending:**")
        for a in pending[-4:]:
            icon = "📝" if "notes" in a.get("type", "") else "📧"
            st.markdown(
                f'<div style="background:#1e293b;border:1px solid #334155;padding:8px 12px;'
                f'border-radius:6px;margin:3px 0;display:flex;justify-content:space-between;">'
                f'<span>{icon} <code>{a.get("type","")}</code></span>'
                f'<span style="background:#b45309;color:#fff;padding:2px 8px;border-radius:8px;font-size:0.72rem">pending</span>'
                f'</div>',
                unsafe_allow_html=True,
            )
        st.info("👉 Open **Approval Center** tab to review and approve")
