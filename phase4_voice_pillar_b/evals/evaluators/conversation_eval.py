...
sys.path.insert(0, str(ROOT / "phase6_pillar_b_voice"))
...
def _make_booking_modules() -> dict:
    ...
    _t1 = datetime(2026, 4, 27, 10, 0, tzinfo=IST)
    _t2 = datetime(2026, 4, 28, 14, 0, tzinfo=IST)
...
def _make_mock_mcp_results(success: bool = True):
    try:
        ...
        return MCPResults(calendar=tr, sheets=tr, email=tr, total_duration_ms=50.0)
    except ImportError:
...
def _run_flow(flow: dict) -> dict[str, Any]:
    ...
    if flow.get("expected_keeps_old_code"):
        keeps_old_code_ok = bool(ctx.booking_code)
